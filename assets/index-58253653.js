...
`}};({entries:ss}=Object);G=["from pathlib import Path as _Path"];un=(e,t)=>{for(const[n,r]of ss(t)){const s=`_Path("${e}/${n}")`;if(typeof r=="string"){const i=JSON.stringify(r);G.push(`${s}.write_text(${i})`)}else G.push(`${s}.mkdir(parents=True, exist_ok=True)`),un(`${e}/${n}`,r)}};un(".",rs);G.push("del _Path");G.push(`
`);const ze=G.join(`
`);const{assign:Ke,defineProperty:pn}=Object;let V;const dn=({tagName:e})=>e==="SCRIPT",wt=e=>{pn(document,"currentScript",{configurable:!0,get:()=>e})},bt=()=>{delete document.currentScript},Q=(e,t,n,r)=>{n(t);for(const s of L[r])s(e,t)};let Et=!0;const os=({XWorker:e,interpreter:t,io:n})=>{function r(...s){const i=e(...s);return i.onerror=({error:o})=>n.stderr(o),i}t.registerJsModule("_pyscript",{PyWorker:r,get target(){return dn(V)?V.target.id:V.id}}),t.runPython(ze,{globals:t.runPython("{}")})},mn={codeBeforeRunWorker:()=>[ze,...L.codeBeforeRunWorker].map(k).join(`
